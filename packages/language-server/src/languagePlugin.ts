...
export type RiotV3VirtualCodeOptions = RiotV3LanguageOptions;

export function createRiotV3LanguagePlugin(
  ...
  return {
    ...
    createVirtualCode(uri, languageId, snapshot) {
      if (languageId === 'riot_v3') {
        return new RiotV3VirtualCode(snapshot, uri.path, options);
      }
    },
    typescript: {
      extraFileExtensions: [
...
export const riotV3LanguagePlugin = createRiotV3LanguagePlugin();
